...
using (var context = new AppDbContext())
{
    ...
    var products = context.ProductEssential.ToList(); //  modelBuilder.Entity<ProductEssential>().ToSqlQuery("Select Name,Price From Products"); DB ye bu sorguyu atar.
    var products2 = context.ProductEssential.Where(x=>x.Price > 200).ToList(); // Burdaki eklediğimiz Where kosulunu
    //  modelBuilder.Entity<ProductEssential>().ToSqlQuery("Select Name,Price From Products"); bulunan SQL cümleciğine ekler ve öyle DB ye sorgu atar , yani gidip
    // Önce veriyi getirip sonrasında Where kosulunu calıstırmaz Where i de ToSqlQuery içerisindeki SQL içerisine ekler öyle gidip DB ye sorgu atar.
}