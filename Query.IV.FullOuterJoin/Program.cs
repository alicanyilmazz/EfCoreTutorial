...
using Query.IV.FullOuterJoin.DAL;
...
using (var context = new AppDbContext())
{
    ...
    //--------------------------------------------------------------------------------------------------------------------//
    var letfJoin = await (from p in context.Products
    ...
    var rightJoin = await (from pf in context.ProductFeature
    ...
    var outerJoin = letfJoin.Union(rightJoin);

    outerJoin.ToList().ForEach(x =>
    {
        Console.WriteLine($"{x.Name} {x.Color} {x.Id}");
    });
