...
using Pagination.DAL;


GetProducts(2, 3).ForEach(x =>
{
    Console.WriteLine("**********************************************");
    Console.WriteLine($"{x.Id} {x.Name} {x.Barcode} {x.tax} {x.Price}");
});

GetProductsWithPriceFilter(2, 3, 100).ForEach(x =>
{
    Console.WriteLine("**********************************************");
    Console.WriteLine($"{x.Id} {x.Name} {x.Barcode} {x.tax} {x.Price}");
});
...
static List<Product> GetProducts(int page, int pageSize)
{
    using (var context = new AppDbContext())
    {
        ...
        // page=1 pageSize=3 => ilk     3 data => skip:0 take: 3
        // page=2 pageSize=3 => sonraki 3 data => skip:3 take: 3
        // page=3 pageSize=3 => sonraki 3 data => skip:6 take: 3
        return  context.Products.OrderByDescending(p => p.Id).Skip((page-1)*pageSize).Take(pageSize).ToList();

    }
}

static List<Product> GetProductsWithPriceFilter(int page, int pageSize,int price)
{
    using (var context = new AppDbContext())
    {
        #region AddDataIfNotExist
        ...
        // page=2 pageSize=3 => sonraki 3 data => skip:3 take: 3
        // page=3 pageSize=3 => sonraki 3 data => skip:6 take: 3
        return context.Products.Where(x=>x.Price>price).OrderByDescending(p => p.Id).Skip((page - 1) * pageSize).Take(pageSize).ToList();
