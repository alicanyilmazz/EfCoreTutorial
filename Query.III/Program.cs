...
using Query.III.LeftAndRightJoin.DAL;
...
using (var context = new AppDbContext())
{
    ...
    // Inner Join kısmında Temelde 2 farklı tip de Linq yazım şeklinden bahsetmiştik orayı inceleyebilirsiniz.
    // Orda Inner Join yaparken kullandıgımız birinci tip Linq Sorgusu Left ve Right Join işlemlerinde bulunmamaktadir.
    // Bu nedenle biz direkt bizimde önermiş oldugumuz ikinci tip Linq sorgu tipi üzerinden Left ve Right Join nasıl yapılabilir onu göstereceğiz.

    // INNERJOIN
    var resultOfInnerJoin = await (from p in context.Products
                 join pf in context.ProductFeature on p.Id equals pf.Id
                 select new {p}).ToListAsync();

    // LEFTJOIN
    var resultOfLeftJoin = await (from p in context.Products
                        join pf in context.ProductFeature on p.Id equals pf.Id into pflist
                        from pf in pflist.DefaultIfEmpty()
                        select new { p,pf }).ToListAsync();

    // LEFTJOIN
    var resultOfLeftJoin2 = await (from p in context.Products
                                  join pf in context.ProductFeature on p.Id equals pf.Id into pflist
                                  from pf in pflist.DefaultIfEmpty()
                                  select new 
                                  {
    ...
    //RIGHTJOIN
    var resultOfRightJoin = await (from pf in context.ProductFeature
